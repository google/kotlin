...
internal object VirtualTablesLookup {
    private fun FunctionGenerationContext.getInterfaceTableRecord(typeInfo: LLVMValueRef, interfaceId: Int): LLVMValueRef {
        ...
        val interfaceTableSize = load2(llvm.int32Type, structGep(typeInfo, 9 /* interfaceTableSize_ */))
        val interfaceTable = load2(interfaceTableRecordPtrType, structGep(typeInfo, 10 /* interfaceTable_ */))
    ...
    fun FunctionGenerationContext.checkIsSubtype(objTypeInfo: LLVMValueRef, dstClass: IrClass) = if (!context.ghaEnabled()) {
        ...
        if (!dstClass.isInterface) {
            ...
            icmpEq(load2(llvm.int32Type, structGep(interfaceTableRecord, 0 /* id */)), llvm.int32(interfaceId))
        }
    }
    ...
    fun FunctionGenerationContext.getVirtualImpl(receiver: LLVMValueRef, irFunction: IrSimpleFunction): LlvmCallable {
        ...
        val llvmMethod = when {
            canCallViaVtable -> {
                ...
                load2(functionPtrType, bitcast(functionPtrPtrType, slot))
            }
                ...
                val vtable = load2(llvm.int8PtrPtrType, structGep(interfaceTableRecord, 2 /* vtable */))
...
internal abstract class FunctionGenerationContext(
    ...
    fun load2(type: LLVMTypeRef, address: LLVMValueRef, name: String = "",
    ...
    fun load(address: LLVMValueRef, name: String = "",
        ...
        return applyMemoryOrderAndAlignment(LLVMBuildLoad(builder, address, name)!!, memoryOrder, alignment)
    }
    ...
    fun loadTypeInfo(objPtr: LLVMValueRef): LLVMValueRef {
        ...
        // TODO(ager): Get rid of the bitcast here by supplying the type in the GEP above.
        val typeInfoOrMetaPtrRaw = bitcast(pointerType(codegen.intPtrType), typeInfoOrMetaPtr)
        val typeInfoOrMetaWithFlags = load2(codegen.intPtrType, typeInfoOrMetaPtrRaw, memoryOrder = memoryOrder)
        ...
        return load2(codegen.kTypeInfoPtr, typeInfoPtrPtr, memoryOrder = LLVMAtomicOrdering.LLVMAtomicOrderingMonotonic)
    }
    ...
    fun getObjCClass(irClass: IrClass, exceptionHandler: ExceptionHandler): LLVMValueRef {
        ...
        return if (irClass.isExternalObjCClass()) {
            ...
            val classPointerGlobal = load2(llvm.int8PtrPtrType, structGep(classInfo, KotlinObjCClassInfoGenerator.createdClassFieldIndex))

            val storedClass = this.load2(llvm.int8PtrType, classPointerGlobal)
    ...
    private fun getObjCClass(binaryName: String) = load2(llvm.int8PtrType, codegen.objCDataGenerator!!.genClassRef(binaryName).llvm)
