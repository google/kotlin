...
class KotlinJvmApiTest : KGPBaseTest() {
    ...
    internal fun kotlinCompilationShouldRunIfSetUpWithApi(gradleVersion: GradleVersion) {
        project(
            ...
            this.projectPath.toFile().resolve("src").let {
                it.deleteRecursively()
                it.resolve("main").mkdirs()
            ...
            val buildFileContent = this.buildGradle.toFile().readText()
            val newContent = buildFileContent.replace("id 'org.jetbrains.kotlin.jvm'", "id 'org.jetbrains.kotlin.jvm' apply false") +
                    """
                ...
                """.trimIndent()
            this.buildGradle.toFile().writeText(newContent)
