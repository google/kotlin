...
class Kapt3GradleSubplugin @Inject internal constructor(private val registry: ToolingModelBuilderRegistry) :
    ...
    private fun Kapt3SubpluginContext.createKaptKotlinTask(useWorkerApi: Boolean): TaskProvider<out KaptTask> {
        ...
        val taskConfiguration = if (taskClass == KaptWithoutKotlincTask::class.java ) {
            KaptWithoutKotlincConfig(taskName, kotlinCompilation.compileKotlinTaskProvider.get() as KotlinCompile, kaptExtension)
        } else {
            KaptWithKotlincConfig(taskName, kotlinCompilation.compileKotlinTaskProvider.get() as KotlinCompile, kaptExtension)
        }

        taskConfiguration.stubsDir.set(getKaptStubsDir())
        taskConfiguration.destinationDir.set(sourcesOutputDir)
        taskConfiguration.kotlinSourcesDestinationDir.set(kotlinSourcesOutputDir)
        taskConfiguration.classesDir.set(classesOutputDir)
        if (javaCompile != null) {
            taskConfiguration.defaultJavaSourceCompatibility.set(javaCompile.map { it.sourceCompatibility })
        }

        if (project.isIncrementalKapt()) {
            taskConfiguration.incAptCache.fileValue(getKaptIncrementalAnnotationProcessingCache()).disallowChanges()
        }
        ...
        taskConfiguration.kaptClasspath.from(kaptClasspathConfiguration).disallowChanges()
        taskConfiguration.kaptExternalClasspath.from(kaptClasspathConfiguration.fileCollection { it is ExternalDependency })
        taskConfiguration.kaptClasspathConfigurationNames.value(kaptClasspathConfigurations.map { it.name }).disallowChanges()

        KaptWithAndroid.androidVariantData(this)?.annotationProcessorOptionProviders?.let {
            taskConfiguration.annotationProcessorOptionProviders.add(it)
        }

        val kaptTaskProvider = if (taskClass == KaptWithoutKotlincTask::class.java) {
            ...
            project.registerTask(taskName, KaptWithoutKotlincTask::class.java, emptyList()).also {
                taskConfiguration.configureTask(it)
            }
        } else {
            taskConfiguration as KaptWithKotlincConfig
        ...
        if (taskClass == KaptWithKotlincTask::class.java) {
            val subpluginOptions = buildOptionsForAptMode(taskConfiguration.javacOptions)
            taskConfiguration.addSubpluginOptions(KAPT_SUBPLUGIN_ID, subpluginOptions)
        } else {
    ...
    private fun Kapt3SubpluginContext.createKaptGenerateStubsTask(): TaskProvider<KaptGenerateStubsTask> {
        ...
        val taskConfiguration =
            KaptGenerateStubsConfig(kaptTaskName, kotlinCompilation, kotlinCompile).also {
                it.stubsDir.set(getKaptStubsDir())
                it.destinationDir.set(getKaptIncrementalDataDir())
                ...
                it.classpathSnapshotDir.fileValue(getKaptClasspathSnapshotDir())
            }

        taskConfiguration.configureTask(kaptTaskProvider)
