...
internal class ReflectiveAccessLowering(
    ...
    private fun recordCompanionObjectAsDispatchReceiver(expression: IrCall) {
        if (((expression.dispatchReceiver as? IrGetField)?.symbol?.owner?.type as? IrSimpleTypeImpl)?.classifier?.descriptor?.isCompanionObject() == true) {
            callsOnCompanionObjects[expression] = (expression.dispatchReceiver!! as IrGetField).symbol.owner.type.classOrNull!!
        }
    }
